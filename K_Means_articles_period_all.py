...
links_012014_06082015=joblib.load('links_012014_06082015.pkl') #load the list of links for Period_1



soup_list=[]

for i in links_012014_06082015: #make a "soup list" using the links 
    soup_list.append(BeautifulSoup(urllib.urlopen(i).read() ,'lxml'))
    
    
def clean(soup): #define a function that will help you with extracting the article content from the soup list 
    content_1=[]
    content_2=[]
    final_content=[]
    for p in soup('p'):
...
article_list=[] #put the articles in a list 
for i in range(len(soup_list)):
    article_list.append("".join(clean(soup_list[i])))
    
# joblib.dump(article_list_012014_06082015,'article_list_012014_06082015.pkl')     
# if you want to, pickle the article list for later use 


# article_list = joblib.load('article_list_period_1.pkl') 
# load the article if you have pickled it 
